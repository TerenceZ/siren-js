...
var debug = require("debuglog")("siren/socket.io/namespace");
...
exports.add = function (client, fn) {
  ...
  if (!this.gen) {
    ...
    this.gen = co.wrap(compose(this.fns.concat([this.router.middleware(), this._onconnect])));
  }

  this.gen.call(socket).catch(function (err) {

    if (client.conn.readyState === "open") {
      if (err instanceof Error) {
        console.error(err.stack);
      }
      socket.socket.error(err.data || err.message || err);
    }
  });
